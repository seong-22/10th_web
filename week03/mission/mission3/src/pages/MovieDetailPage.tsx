...
const MovieDetailPage = () =>{
            ...
            try{
                const [detailRes, creditsRes] = await Promise.all([
                    axios.get(`https://api.themoviedb.org/3/movie/${movieID}?language=ko-KR`, {
                        headers: 
                        { Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiOGYzMzY5NDcwMDY0YTBmNzk0ZTkwMmYxZjA2ZThhOCIsIm5iZiI6MTc3NDUxMTE5MS4wNTIsInN1YiI6IjY5YzRlNDU3NWMwZGIzZDkwZjdjYWNkNiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.LqkHpEUYnUSGSSoKovIF0yuzo61co7dTDSvbFl6u1Y4` }
                    }),
                    axios.get(`https://api.themoviedb.org/3/movie/${movieID}/credits?language=ko-KR`, {
                        headers: { Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiOGYzMzY5NDcwMDY0YTBmNzk0ZTkwMmYxZjA2ZThhOCIsIm5iZiI6MTc3NDUxMTE5MS4wNTIsInN1YiI6IjY5YzRlNDU3NWMwZGIzZDkwZjdjYWNkNiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.LqkHpEUYnUSGSSoKovIF0yuzo61co7dTDSvbFl6u1Y4` }
                    })
                ]);
    ...
    return (
    <div className="min-h-screen bg-[#0f172a] text-white p-8"> {/*전체 배경화면 색 설정, 글자 색상 설정*/}
                    ...
                    {credits?.cast?.slice(0, 10).map((person: any) => (
                        <div key={person.id} className="min-w-[120px] text-center">
                            ...
                            <div className="w-24 h-24 mx-auto mb-3 rounded-full overflow-hidden bg-slate-800 border-2 border-slate-700">
                                <img 
                                    src={person.profile_path ? `https://image.tmdb.org/t/p/w185${person.profile_path}` : 'https://via.placeholder.com/150'} 
